...
@section('header')
    <div class="flex items-center gap-4">
        <a href="{{ route('admin.students.show', $student) }}" class="text-gray-500 hover:text-gray-700 transition">
            <i class="fa-solid fa-arrow-left text-xl"></i>
        </a>
        <span>Edit Data: {{ $student->nama_lengkap }}</span>
    </div>
@endsection
...
@section('content')
    ...
    {{-- --- CEK STATUS READ ONLY DIHAPUS UNTUK ADMIN --- --}}
            ...
            <fieldset>
                ...
                <div x-show="step === 1" x-transition:enter="transition ease-out duration-300"
                    ...
                    <div class="grid grid-cols-1 md:grid-cols-2 gap-6 mb-6">
                        {{-- Status Siswa --}}
                        <div>
                            <label class="block text-sm font-bold text-gray-700 mb-1">Status Siswa <span class="text-red-500">*</span></label>
                        ...
                        <div>
                            @php
                                $p_seq = ''; $p_month = ''; $p_year = '';
                                if($student->participant_number && preg_match('/^HGS\/(\d+)\/([A-Z]+|\d+)\/(\d+)$/', $student->participant_number, $matches)){
                                    $p_seq = $matches[1];
                                    $p_month = $matches[2];
                                    $p_year = $matches[3];
                                }
                            @endphp
                            <label class="block text-sm font-bold text-gray-700 mb-1">Nomor Peserta (HGS/.../.../...)</label>
                            <div class="flex items-center gap-2">
                                <span class="bg-gray-100 border border-gray-300 px-3 py-2 rounded-lg text-gray-600 font-bold">HGS/</span>
                                <input type="text" name="p_seq" value="{{ old('p_seq', $p_seq) }}" placeholder="001" class="w-20 rounded-lg border-gray-300 text-center" maxlength="4">
                                <span class="font-bold text-gray-400">/</span>
                                <input type="text" name="p_month" value="{{ old('p_month', $p_month) }}" placeholder="IV" class="w-16 rounded-lg border-gray-300 text-center" maxlength="5">
                                <span class="font-bold text-gray-400">/</span>
                                <input type="text" name="p_year" value="{{ old('p_year', $p_year) }}" placeholder="2026" class="w-20 rounded-lg border-gray-300 text-center" maxlength="4">
                            </div>
                            @error('participant_number') <p class="text-red-500 text-xs mt-1 font-bold">{{ $message }}</p> @enderror
                        </div>
                    </div>
