...
@section('header', 'Edit Pegawai: ' . $employee->nama)
...
<div class="flex justify-between items-center mb-6">
    <a href="{{ route('admin.employees.index') }}"
       class="inline-flex items-center px-4 py-2 bg-white text-gray-700 border border-gray-300 rounded-xl hover:bg-gray-50 transition font-bold shadow-sm">
       <i class="fa-solid fa-arrow-left mr-2"></i> Kembali
    </a>
    <a href="{{ route('admin.employees.export-biodata', $employee->id) }}" target="_blank"
...
<div class="grid grid-cols-1 lg:grid-cols-3 gap-8" x-data="{ activeTab: 'biodata' }">
    ...
    <div class="lg:col-span-2">
                ...
                <div class="space-y-4">
                    ...
                    <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                        ...
                        <div>
                            ...
                            <input type="text" name="jabatan" value="{{ old('jabatan', $employee->jabatan) }}" class="w-full border-gray-300 rounded-lg focus:border-gold-500">
                        </div>
                    </div>
                    ...
                    <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                        <div>
                            ...
                            <input type="text" name="nip" value="{{ old('nip', $employee->nip) }}" class="w-full border-gray-300 rounded-lg focus:border-gold-500">
                        </div>
                        <div>
            ...
            <div class="space-y-4">
                @forelse($employee->educations as $edu)
                    <div class="flex justify-between items-start p-4 bg-gray-50 rounded-xl border border-gray-100">
                        ...
                        <form action="{{ route('admin.employees.education.destroy', [$employee->id, $edu->id]) }}" method="POST" onsubmit="return confirm('Hapus data pendidikan ini?')">
                            @csrf @method('DELETE')
                        ...
                        @forelse($employee->families as $fam)
                            <tr>
                                ...
                                <td class="px-4 py-3 text-right whitespace-nowrap">
                                    ...
                                    <form action="{{ route('admin.employees.family.destroy', [$employee->id, $fam->id]) }}" method="POST" class="inline" onsubmit="return confirm('Hapus data keluarga ini?')">
                                        @csrf @method('DELETE')
        ...
        <div x-show="activeTab === 'dokumen'" class="bg-white p-6 rounded-2xl border border-gray-200 shadow-sm animate-fade-in" style="display: none;">
            ...
            {{-- Form Upload Dokumen --}}
            <form action="{{ route('admin.employees.document.store', $employee->id) }}" method="POST" enctype="multipart/form-data" class="bg-blue-50 p-4 rounded-xl border border-blue-100 mb-6">
                @csrf
                ...
                @forelse($employee->documents as $doc)
                    <div class="flex items-center justify-between p-3 bg-white border border-gray-200 rounded-lg hover:border-gold-300 transition shadow-sm">
                        ...
                        <div class="flex gap-2 shrink-0">
                            ...
                            <form action="{{ route('admin.employees.document.destroy', [$employee->id, $doc->id]) }}" method="POST" onsubmit="return confirm('Hapus dokumen ini?')">
                                @csrf @method('DELETE')
...
<div id="modalPendidikan" class="fixed inset-0 bg-black bg-opacity-50 flex justify-center items-center hidden z-50 p-4">
    <div class="bg-white w-full max-w-md rounded-xl shadow-lg p-6">
        ...
        <form action="{{ route('admin.employees.education.store', $employee->id) }}" method="POST">
            @csrf
            <div class="space-y-3">
                <div>
...
<div id="modalKeluarga" class="fixed inset-0 bg-black bg-opacity-50 flex justify-center items-center hidden z-50 p-4">
    <div class="bg-white w-full max-w-md rounded-xl shadow-lg p-6">
        ...
        <form action="{{ route('admin.employees.family.store', $employee->id) }}" method="POST">
            @csrf
            <div class="space-y-3">
                <div>
...
<script>
    ...
    function openEditKeluarga(data) {
        // Set Action URL
        let url = "{{ route('admin.employees.family.update', ['employee' => $employee->id, 'id' => ':id']) }}";
